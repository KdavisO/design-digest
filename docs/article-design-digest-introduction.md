...
DesignDigest は、この「気づけない」問題を自動化で解決するツールだ。GitHub Actions の cron で平日1日1回 Figma ファイルをチェックし、前回との差分を検出して Slack に通知する。スケジュールは cron 式で任意に変更できる。
...
初回のベースライン作成時を除き、変更がない場合も「変更なし」の通知を送る。これにより「通知が来ないのは変更がないからか、ツールが壊れているからか」の不安を解消している。
...
DesignDigest は「デザイナーに何かを求めるのではなく、開発者側の仕組みで解決する」というアプローチを取っている。Figma のファイルを平日静かにチェックし、変更があれば知らせる。ただそれだけのツールだ。
