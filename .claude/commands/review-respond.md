...
**波及チェック（修正前に必ず実施）**: 各コメントの修正に着手する前に、指摘されたパターンがプロジェクト内の他の箇所にも存在しないかを確認する。これにより「修正→新たな指摘→再修正」の連鎖を防止する。

1. **同一パターンの検索**: 指摘対象の表現・構造・コマンドパターンを grep 等で検索し、同一パターンの他の出現箇所を列挙する
   - 例: `if ...; then ...; fi` の終了コード問題 → 同じパターンを使っている全ファイルを検索
   - 例: ファイルパスの参照表記（フルパス vs 短縮名）→ 同一ドキュメント内と相互参照ファイルの表記を統一
2. **cross-referenceファイルの確認**: 修正対象ファイルを参照・引用している他のファイルに、同じパターンや矛盾する記述がないか確認する
   - 例: `SETUP.md` と `.claude/rules/` 内のガイドラインが同一設定例を記載している場合、両方を修正する
3. **一括修正**: 列挙された全箇所を一括で修正してからコミットする（部分修正による再指摘を防止）
...
1. Copilot再レビューリクエストがpendingでないことを確認する:
   ...
   - 1以上（Copilotがまだrequested_reviewersに残っている = 再レビュー未完了）:
     - **自動マージは実行しない**
     - 「Copilot再レビューが未完了のためマージを保留します。手動マージは行わず、再レビュー完了後にポーリングのポストアクションを継続/再開するか、必要に応じて `/review-respond --auto` を再実行してください」とサマリーに記録し、この時点でポストアクションを終了する
