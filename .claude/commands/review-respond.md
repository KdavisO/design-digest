...
- **カウンターファイル**: `/tmp/design-digest-review-{ownerRepo}-idle-{PR番号}`
  - 内容: 連続空振り回数（整数値のみ）
...
1. **空振り時**（「スキップ:」で即終了するケースのうち、**未対応コメントなし**・**Copilotレビュー未着** の場合のみをカウント対象とする）:
   - カウント対象の理由でスキップした場合:
     ...
     - カウンター値 >= N の場合:
       - cronタスクIDファイル（`/tmp/design-digest-review-{ownerRepo}-cron-{PR番号}`）を読み取り、タスクIDを取得
...
2. **対応実行時**（コード修正・コメント返信を実施したケース）:
   - カウンターファイルをリセット（`echo 0 > /tmp/design-digest-review-{ownerRepo}-idle-{PR番号}`）
   - 通常の完了報告を出力
...
- **いずれかが `state: PENDING` または `state: IN_PROGRESS`**: 「スキップ: CI実行中」と出力して**即終了**
...
1. スコープ外Issue候補ファイル（`/tmp/design-digest-review-{ownerRepo}-deferred-{PR番号}`）を読み取る
